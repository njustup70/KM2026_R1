...
void Action_Planning(StateCore *state_core)
{
    ...
    // 等待遥控器确认KFS数据已发好
    Seq::WaitUntil([]() -> bool { return farcon.button_second_half[9 - 8 - 1] == 1; });
    Zone2_Path.index = 0;
...
void Action_NavToBlock(StateCore *state_core)
{
	...
	if (is_ready_to_pick | is_final_goal_reached) return; // 如果已经触发取块，直接返回，等待状态切换,防止线程时序问题没有正确跳转


    // 判断是否已走完全部路径点
    if (Zone2_Path.index >= Zone2_Path.size)
    {
        is_final_goal_reached = true; // 触发 LinkTo 切到三区 prelay
        return;
    }

    // 提取当前这一帧动作节点的打包数据
    PathNode& current_node = Zone2_Path.nodes[Zone2_Path.index];
    Vec2 target_pos = current_node.pos;
    int target_xid = current_node.label;
    float target_yaw = current_node.target_yaw;
    ...
    {
        // 强等待底盘横移就位
        Seq::WaitUntil([]() -> bool { return (chassis._Walking() == 1); });
        
        // 调整yaw
        chassis.RotateAt(target_yaw); 
        
        // 强等待旋转就位
        Seq::WaitUntil([]() -> bool { return (chassis._Rotating() == 1); });
    }
    else
    {
        // 普通通过点，也只需要等底盘横移到达即可
